...
[{else}]
    [{assign var="params" value=$oViewConf->getSelfLink()|oxaddparams:"cl="|cat:$oViewConf->getActiveClassName()|cat:"&fnc=getexternpictures&d3ief=`$iImportedFiles`&oxid=`$oxid`"}]
...
<form name="myedit" id="myedit" enctype="multipart/form-data" action="[{$oViewConf->getSelfLink()}]" method="post">
    ...
    <table cellspacing="0" cellpadding="0" border="0" width="98%">
        ...
        [{if false == $iError && false == $iSumFiles}]
            [{assign var="iError" value="20"}]
        [{/if}]
        [{if $iError}]
            [{block name="d3_d3importer_views_admin_tpl_d3_picture_extticker_failure"}]
                <tr>
        ...
        [{else}]
            [{block name="d3_d3importer_views_admin_tpl_d3_picture_extticker_success"}]
                <tr>
                    <td valign="top" class="edittext">
                        <div style="text-align: center;">
                            <h3>[{oxmultilang ident="D3_PICTURE_EXTIMPORT_INFO1"}]
                                : [{$iImportedFiles}] [{oxmultilang ident="D3_PICTURE_IMPORT_INFO2"}] [{$iSumFiles}]
                                .</h3>
                            [{if $blEOD}]
                                <h3>[{oxmultilang ident="D3_PICTURE_EXTIMPORT_IMPORTEND"}]</h3>
                                [{include file="d3_importer_application_views_admin_tpl_options_picture_import.tpl" showStartButton=$showStartButton|default:false showScanArea=false}]
                            [{/if}]
